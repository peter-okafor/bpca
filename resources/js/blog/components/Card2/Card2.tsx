...
const Card2: FC<Card2Props> = ({
  ...
  return (
    <div
      className={`nc-Card2 group relative flex flex-col rounded-lg [ nc-box-has-hover ] [ nc-dark-box-bg-has-hover ] overflow-hidden ${className}`}
      data-nc-id="Card2"
    >
      ...
      <Link href={`${process.env.MIX_BLOG_URL}${href}`} className="absolute inset-0" />
      ...
      <div className="p-4 sm:p-5 flex flex-col">
        <div className="space-y-3">
          ...
          >
            <Link href={`${process.env.MIX_BLOG_URL}${href}`} className="line-clamp-2" title={title}>
              {title}
            </Link>
          </h2>
